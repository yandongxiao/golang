...
//
// We can view each interface value as a box to encapsulate
// a non-interface value. To box/encapsulate a non-interface
// value into an interface value, the type of the non-interface
...
//  so the time complexity of the copy is O(1), instead of O(n).
//
...
//
// If the method set of an arbrtrary type T, T may be an interface
// type or not, is a super set of the method set of an interface type I,
// then we say type T implements interface I.
...
func main() {
	...
	// Two unnamed interface types are identical if their method sets are
	// identical. Please note, non-exported method names, which start with
	// lower-case letters, from different packages will be always viewed as
	// two different method names, even if the two method names are the same in literal.
	var i interface{}
}
