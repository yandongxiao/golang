...
flat: Sort entries based on own weight (默认情况)
Each stack sample only includes the bottom 100 stack frames: 解释main.main函数的第五列为啥不是100%
(pprof) top5 -cum
Total: 2525 samples
...
func main() {
	...
	if *profile != "" {
		...
		if err != nil {
			log.Fatal("failed to start cpu profile: %v", err)
		}
